...
import { canPostToZapierWebook, postToZapierWebook } from "../lib/zapier";
...
const rootMutations = {
  RootMutation: {
    ...
    addTagsToCampaignContacts: async (
      ...
      if (canPostToZapierWebook()) {
        try {
          campaignContactIds.forEach(async campaignContactId => {
            const contact = await loaders.campaignContact.load(campaignContactId);
            const campaign = await loaders.campaign.load(contact.campaign_id);
            ...
            // not awaiting so we don't block
            postToZapierWebook(payload);
          });
        } catch (error) {
          console.error(`Error posting to Zapier webhook ${error}`);
        }
      }
